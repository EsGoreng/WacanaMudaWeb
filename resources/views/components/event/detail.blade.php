...
<div class="flex flex-col md:flex-row h-full md:h-[600px]">
    ...
    <div class="w-full md:w-1/2 p-8 md:p-10 flex flex-col h-full bg-zinc-800 relative overflow-y-auto">
        ...
        <div class="flex items-center text-zinc-950 dark:text-zinc-300 mb-4 text-sm">
            ...
            @else
                <x-bi-geo-alt class="mr-2 h-5 w-5 flex-shrink-0" />
                <span>
                    {{ $event->location_name }}
                    @if ($event->location_address)
                        <span class="text-zinc-500 dark:text-zinc-400">, {{ $event->location_address }}</span>
                    @endif
                </span>
            @endif
        </div>
        ...
        <div class="mt-auto pt-6 border-t border-zinc-100 dark:border-zinc-700 flex flex-col gap-3">
            ...
            @if ($event->is_online && $event->meeting_link)
                <a href="{{ $event->meeting_link }}" target="_blank" rel="noopener noreferrer" class="w-full">
                    <flux:button variant="primary"
                        ...
                        <x-bi-camera-video class="w-4 h-4" />
                        Join Event
                    </flux:button>
                </a>
            @elseif($event->register_link)
                <a href="{{ $event->register_link }}" target="_blank" rel="noopener noreferrer" class="w-full">
                    <flux:button variant="primary"
                        class="bg-brand-hover hover:bg-accent text-white font-semibold py-2 px-3 rounded-lg transition-all duration-200 flex items-center justify-center gap-2 text-sm w-full">
                        Register
                    </flux:button>
